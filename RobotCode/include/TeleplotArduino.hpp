...
// Modified by MiAM Robotics to allow use in Arduino with WiFiUdp
...
public:
    class ShapeValue
    {
        public :
            ...
            ShapeValue(float val) : isSet(true), value(val) {};
            ...
            private :
                std::string roundValue(const float value, const unsigned short precision) const
                {
...
public:
    ...
    void update(std::string const& key, T const& value, std::string unit = "", unsigned int maxFrequencyHz = 0, std::string flags = TELEPLOT_FLAG_DEFAULT) {
        ...
        float nowMs = static_cast<float>(nowUs)/1000.f;
...
private:
    ...
    void emit(std::string const& data){
        ...
        udp.beginPacket(address_.c_str(), port_);
        size_t size = strlen(data.c_str());
        udp.write((uint8_t*)(data.c_str()), size);
        udp.endPacket();
    }
