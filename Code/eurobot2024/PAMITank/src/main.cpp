...
#define REMOTE_DEAD_ZONE 450
#define REMOTE_MAX_VALUE 2048.0
...
AbstractRobotBase* robotBase = RobotBaseDC::getInstance();
...
void loop()
{
  ...
  if (newMessageReceived)
  {
    ...
    remoteStickReading[0] =  myData.X;
    remoteStickReading[1] =  myData.Y;
  ...
  if (currentControlState == ControlState::CONTROLLED_BY_REMOTE)
  {
    if (millis() - lastRemoteControllerMillis > REMOTE_CONTROLLER_TIMEOUT)
    {
      ...
      // wheelSpeed_rad_s_[RIGHT_ENCODER_INDEX] = 0;
      // wheelSpeed_rad_s_[LEFT_ENCODER_INDEX] = 0;
      driveTrainTarget.motorSpeed[0] = 0.0;
      driveTrainTarget.motorSpeed[1] = 0.0;
      stepperCommunicationState = StepperCommunicationState::COMMAND_REQUESTED;
    }
    else
    {
      if (needSendCommand)
      {
        if (remoteStickReading[0] > REMOTE_DEAD_ZONE || remoteStickReading[1] > REMOTE_DEAD_ZONE)
        {
          ...
          newBaseSpeed = BaseSpeed(
            maxLinearBaseSpeed * x,
            maxAngularBaseSpeed * y // if joystick pushed left, angle should be direct
          ) ;

          WheelSpeed newWheelSpeed = driveTrainKinematics.inverseKinematics(newBaseSpeed);
          driveTrainTarget.motorSpeed[0] = newWheelSpeed.left;
          driveTrainTarget.motorSpeed[1] = -newWheelSpeed.right;
          ...
          log_d(
            "driveTrainTarget.motorSpeed[0] %f - driveTrainTarget.motorSpeed[1] %f", 
            driveTrainTarget.motorSpeed[0],
            driveTrainTarget.motorSpeed[1]
          );
        }
        else
        {
          newBaseSpeed = BaseSpeed(
            0.0,
            0.0 // if joystick pushed left, angle should be direct
          ) ;
          driveTrainTarget.motorSpeed[0] = 0.0;
          driveTrainTarget.motorSpeed[1] = 0.0;
          stepperCommunicationState = StepperCommunicationState::COMMAND_REQUESTED;
  ...
  if (stepperCommunicationState == StepperCommunicationState::COMMAND_REQUESTED)
  {
    if (currentMotorState == MotorState::POWERED)
    {
      ...
      // );
      log_i(
        "SET driveTrainTarget.motorSpeed[0] %f - driveTrainTarget.motorSpeed[1] %f", 
        driveTrainTarget.motorSpeed[0],
        driveTrainTarget.motorSpeed[1]
      );
      robotBase->setBaseSpeed(driveTrainTarget);
      bool motorEnabled = !(driveTrainTarget.motorSpeed[0] == 0 && driveTrainTarget.motorSpeed[1] == 0);
      robotBase->updateControl(motorEnabled);
    }
    else
    {
      driveTrainTarget.motorSpeed[0] = 0.0;
