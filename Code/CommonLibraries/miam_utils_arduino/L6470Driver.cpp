...
namespace miam
{
    ...
    L6470::L6470(SPIWrapper *spiDriver, int const& numberOfDevices):
        ...
        numberOfDevices_(abs(numberOfDevices)),
        stepModeMultiplier_(1.0)
    {
    ...
    void L6470::setParam(uint8_t const& param, std::vector<uint32_t> const& parameterValues)
    {
        ...
        for(uint8_t i = 0; i < numberOfDevices_; i++)
    ...
    std::vector<float> L6470::getPosition()
    {
    ...
    std::vector<float> L6470::getSpeed()
    {
    ...
    void L6470::setSpeed(std::vector<float> const& motorSpeeds)
    {
        std::vector<uint8_t> commands;
        ...
        for(uint8_t i = 0; i < numberOfDevices_; i++)
        {
            ...
            // Register value.
            uint32_t registerValue = (abs(speed) * STEPSEC_TO_VELOCITY_REG) + 0.5;
    ...
    void L6470::setVelocityProfile(float const& maxSpeed, float const& accel, float const& decel)
    {
        sendCommand(dSPIN_SOFT_HIZ);
    ...
    void L6470::moveNSteps(std::vector<float> nSteps)
    {
        std::vector<uint8_t> commands;
        std::vector<uint32_t> parameters;
        // Fill command and parameters registers.
        for(uint8_t i = 0; i < numberOfDevices_; i++)
        {
            ...
            // Register value.
            uint32_t registerValue = static_cast<uint32_t>(abs(nSteps[i] * stepModeMultiplier_));
            // Clamp
    ...
    std::vector<uint32_t> L6470::sendCommand(std::vector<uint8_t> const& commands, std::vector<uint32_t> const& parameters, uint8_t paramLength)
    {
        ...
        {
            for(uint8_t j = 0; j < numberOfDevices_; j++)
        ...
        {
            spiCtrl[x].tx_buf        = &data[2 * x];
            spiCtrl[x].rx_buf        = &data[2 * x];
